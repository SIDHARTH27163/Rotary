...
<head>
  ...
  <title>  AVS :CP | Rotary Club Of Dharamshala</title>
...
$sql = "select * from avs_session where id='$dmbid'";
$rs_result = mysqli_query ($connection,$sql);
?>
<?php
while ($row = mysqli_fetch_assoc($rs_result)) {
?>
<?php $dmb_session=$row["avs_session"]; ?>
<?php };?>

          <div class="card">
            <div class="card-header bg-dark">
              <h3 class="card-title">Add AVS</h3>
              <a href="viewavs.php" class="float-right"><input type="submit" name="view_designated" value="View AVS" class="btn btn-primary"></a>
            </div>
            <!-- /.card-header -->
              ...
              if(isset($_POST["Add"]))
{
      ...
      $query = "INSERT INTO avs(av_id,aw_session,name,image) VALUES ('".$dm_id."','".$dm_session."','".$name."', '".$file_name."')";
      echo $query;
      ...
      $location = "uploads/admin/Gallery/avs/";

                         
                       // moving the file to specific folder
                       move_uploaded_file($file_tmp,$location.$file_name);

                        echo " <div class='alert alert-success mydiv'>AVS NAME:  $name Successfully Added.</div>";

    }
    else{
     
     echo " <div class='alert alert-danger mydiv'><strong>AVS Name not added!</strong ></div>";
...
?>
        ...
        <label> Name</label>
        <input type="text" name="name" placeholder="Enter AVS Name" class="form-control" required>
        <p class="help-block" style="font-weight:italic">Example : "John Wick"</p>
          
        </div>

        <div class="form-group">
        <label>AVS IMAGE</label>
